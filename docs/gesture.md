...
# 把 active 範例設為 handgesture(video_example_media_framework.c 的 example_mmf2_video_surport):
...
> 手勢與臉部辨識是**不同的 NN 範例**(單 NPU 一次跑一個),所以是「手勢模式」或「臉鎖模式」二選一的韌體;要同時用需另外設計(超出目前範圍)。
